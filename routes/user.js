...
router.post("/signup", (req, res) => {
  body("email", "Email is invalid. Please input a valid email address.")
    .exists()
    .isEmail();
  body(
    "password",
    "Password is invalid (empty or length less than 8 characters)."
  )
    .notEmpty()
    .isLength({ min: 8 });
  ...
  User.getOneEmail(req.body.email).then((user) => {
    if (!user) {
      ...
      res
        .status(400)
        .json({ message: "Email already exist. Please choose another email." });
    }
  });
});
...
router.post("/resetpassword", async (req, res) => {
  ...
  if (user) {
    ...
    res
      .status(400)
      .json({ message: "Email doesn't exist. Please fill in a valid email." });
  }
});
